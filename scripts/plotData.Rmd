...
```{r read_data}
motif_df = read.csv('motif_counts.csv',
                    header=T)
length_df = read.csv('phage_lengths.csv', 
                     header=T)
rownames(length_df) = length_df$phage
table_four_df = read.csv('table-four.csv',
                         header=T)
rownames(table_four_df) = table_four_df$phage
```

```{r total_counts}
test_motif_df = motif_df[which(motif_df$phage=="JuraSubmitted"),]
...
table_four_df.melt$variable = ordered(table_four_df.melt$variable,
                                      levels=c("pBrxXL.Sty", "pBrxXL.Eferg", "pBREX.AL"),
                                      labels=c("pBrxXL(Sty)", "pBrxXL(Eferg)", "pBREX-AL"))
...
p.combined = cowplot::plot_grid(p.EOP, 
                                p.motif.histogram,
                   nrow=1,
                   align='h', axis='bt',
                   rel_widths=c(0.3, 1.2))
...
* Unified font sizes esp. in first panel. DONE
* Subscript and italics as in the original table... DONE
* Underline the 5th A in each of the motifs... DONE
* Write materials and methods and figure legend DONE

* Gaps between groups as in the original table. DONE
* Put font into Arial. DONE 
* Justify the boxes between left and right panels. DONE
* Add y-axis scale for the histograms. DONE 
* Sort out the grey boxes for the phage: make it be on the bottom of the histogram, so histogram sits on top. DONE
* Add pink level for Mav Brex-AL: 1.05 x10-4 to adjust the key, so that red is <1E-5. And change so it's properly formatted. DONE
* Colour histogram bars by the height of the bar e.g. in red. DONE
...
```{r combine_plots}
p.combined = cowplot::plot_grid(p.EOP+theme(legend.position = "left")+
